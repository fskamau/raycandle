...
class CustomLive:
    ...
    def __init__(self,df:pd.DataFrame):
        ...
        collector=raycandle.Collector(fig,
                                  fig.ax[0].plot(raycandle.Candle(df[cfc.COLUMNS])),
                                  [fig.ax[0].plot(x, label_from_data=True) for x in cfc.bollinger_bands(df["c"])],
                                  ...
                                  [fig.ax[3].plot(df[x], label_from_data=True) for x in ["+dmi", "-dmi", "adx"]]
                                  )
        fig.show_legend()
        fig.set_title("simple Live data update simulation with raycandle.Collector and fig.show(block=False)")
        fig.show_cursors()
        fig.show(False)#no blocking

        for index in range(CustomLive.SHOWONLY+1,len(df_copy)):
            if fig.is_window_closed:break
