...
void locator_tooltip_mouse_position(Axes *axes, Str buffer, int mouseX,
                                    int mouseY);
/**
formats `epoch` to strftime using `format` and writes formatted time into
`buffer`. Fails if formatted time length exceeds `len_buffer` if `epoch` is 0
`epoch` is set to current epoch. if `buffer` is NULL, formatted time is sent to
stdout.
*/
