...
static bool pre_border_draw_adjust_axes_dimensions(Axes *axes);//plots the axes frames and ajusts the axes dimensions
...
static void axes_draw_title(Axes* axes){
  if (axes->title!=NULL){
    float x=align_text(FIGURE_FONT(axes->parent),axes->title,axes->width,
		       axes->parent->font_size,axes->parent->font_spacing,RC_ALIGNMENT_CENTER);
    DrawTextEx(FIGURE_FONT(axes->parent),axes->title,(Vector2){
	axes->startX+x,axes->startY-axes->parent->font_size},axes->parent->font_size,axes->parent->font_spacing,axes->parent->text_color);
    DrawLineEx((Vector2){axes->startX,axes->startY},(Vector2){
	axes->startX+axes->width,axes->startY},AXES_FRAME_THICK,axes->parent->axes_frame_color);
  }
}
...
void create_axes(Figure *figure,char* labels){
  ...
  for (size_t i=0;i<figure->axes_len;++i){
    ...
    .artist_len=0,
    .padding=0.f,
    .ylocator=(Locator){.format="%.5f",.flen=11,.limit=(Limit){.is_static=false},.ftype=FORMATTER_LINEAR_FORMATTER},
...
bool draw_axes(Axes* axes){
  ...
  if(!pre_border_draw_adjust_axes_dimensions(axes)){return false;}
  if(!RC_COLOR1_EQUALS_COLOR2(axes->facecolor,axes->parent->background_color)){
    DrawRectangle(axes->startX,axes->startY,axes->width,axes->height,axes->facecolor);} //save some fps
  if(axes->legend.legend_position!=LEGEND_POSITION_NO_LEGEND){axes_draw_legend(axes);}
  axes_draw_title(axes);
  if(!post_border_draw_adjust_axes_dimensions(axes)){return false;}
...
void axes_set_title(Axes* axes,char * title){  
  if(axes->title==NULL)axes->title=string_create_from_format(0,NULL,"%s",title);
  else string_destroy(axes->title);
