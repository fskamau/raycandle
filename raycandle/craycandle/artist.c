...
static void artist_line_init(Artist* artist,void*config){
  ...
  case LINE_TYPE_V_LINE:{
    RC_ASSERT(artist->gdata.ydata&&artist->gdata.ydata[0]>=0&&artist->gdata.ydata[0]<=1);
    line_data_->data=cm_malloc(sizeof(*line_data->data)*2,RC_ECHO(line_data_->data));
  ...
  if(!artist->color){*color=axes_get_next_tableau_t10_color(artist->parent);}
  else{memcpy(color,artist->color,sizeof(Color));}
  artist->color=color;
}
...
static void artist_candle_init(Artist* artist,void* config){
  ...
  */
  ...
  candledata->color_indexes=cm_malloc(sizeof(*candledata->color_indexes)*vdata,RC_ECHO(CandleData.color_indexes));
...
static void artist_line_update_data_buffer(Artist* artist,LimitChanged lim){
  if(lim==LIMIT_CHANGED_XLIM){return;}// user changing xlim does no change ylim
...
static void artist_candle_update_data_buffer(Artist* artist,LimitChanged lim){
  ...
  for(size_t i=0;i<vdata;++i,++yindex){
    ...
    if(lim==LIMIT_CHANGED_YLIM||lim==LIMIT_CHANGED_ALL_LIM){
      ...
      candledata->p0[i]=RC_DATA_Y_2_PIXEL(cdata[ldata+yindex],artist->parent);
      candledata->p1[i]=RC_DATA_Y_2_PIXEL(ogtc?cdata[yindex]:cdata[ldata*3+yindex],artist->parent);
      candledata->p2[i]=RC_DATA_Y_2_PIXEL(!ogtc?cdata[yindex]:cdata[ldata*3+yindex],artist->parent);
      candledata->p3[i]=RC_DATA_Y_2_PIXEL(cdata[ldata*2+yindex],artist->parent);
...
static void artist_line_plot(Artist *artist) {
  ...
  case LINE_TYPE_H_LINE:
    if(isnan(line_data.data[1]))goto label_non_segmented_line_having_nan;
    DrawLineEx((Vector2){artist->parent->startX, line_data.data[1]}, (Vector2){artist->parent->startX + artist->parent->width, line_data.data[1]}, artist->thickness, *artist->color);
    return;
  case LINE_TYPE_V_LINE:
    if(isnan(line_data.data[1]))goto label_non_segmented_line_having_nan; 
    DrawLineEx((Vector2){line_data.data[1], artist->parent->startY}, (Vector2){line_data.data[1], artist->parent->startY + artist->parent->height}, artist->thickness, *artist->color);
    return;
  default:
 ...
 label_non_segmented_line_having_nan:{
    ...
    RC_ERROR("non-segmented line unreachable condition .plotting exactly 1 point %f on axes with limits (min=%f, max=%f)\n",line_data.data[0],limit.limit_min,limit.limit_max);
  }
}
...
static void artist_line_draw_icon(Artist* artist,Vector2 startPos){
  startPos.y+=artist->parent->parent->font_size/2.f; //draw a line middle as icon
  DrawLineEx(startPos, (Vector2){startPos.x+RC_LEGEND_ICON_WIDTH,startPos.y},1,*artist->color);
}

static void artist_candle_draw_icon(Artist* artist,Vector2 startPos){
  int y=artist->parent->parent->font_size/3; // draw a half rect as icon
  DrawRectangleLines(startPos.x,startPos.y+y,RC_LEGEND_ICON_WIDTH,artist->parent->parent->font_size-y*2,artist->color[0]);
}

inline Artist* get_artist(Axes*axes,size_t index){
  if((long int)index<0||index+1>axes->artist_len)RC_ERROR("requested artist index %zu but axes only has %zu items\n",index,axes->artist_len);
   Artist *artist=axes->artist;
   for(size_t i=0;i<index;++i)artist=artist->next;
   return artist;
}
...
Artist *create_artist(Axes* axes,ArtistType artist_type,Gdata gdata,double ydata_minmax[2],float thickness,Color* color,void* config){
  ...
  *artist=(Artist){
  ...
  if(gdata.label!=NULL){RC_ASSERT(gdata.label[0]!='\0');}
  if(axes->artist_len==0){
    RC_ASSERT(axes->artist==NULL);
    axes->artist=artist;
    }
  else{get_artist(axes,axes->artist_len-1)->next=artist;}
  if(isnan(ydata_minmax[0])||isnan(ydata_minmax[1])){RC_ERROR("create_artist failed to create Artist(name='%s'). Data limits cannot contain NaN\n",(artist->gdata.label==NULL?"Nameless":artist->gdata.label));}
