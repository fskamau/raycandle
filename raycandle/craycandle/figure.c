...
static bool
set_real_span_skel_map(Figure *figure);   // map axes skeletons to real pixels
...
static void
draw_current_time(Figure *figure); // show current time. might be used if the
  ...
  *cskel = (struct Cskel){0};
  ...
  for (size_t srow = 0; srow < row; ++srow) {
    ...
    for (size_t scol = 0; scol < cols; ++scol) {
      ...
      if (cc != ca)
        SKEL_ERROR("skipped occurence; consecutive col occurence of char "
                   "%c is %d but total occurence is %d in row %zu\n",
                   c, cc, ca, srow);
      struct Cskel otemp = (struct Cskel){
          .sx = scol, .sy = srow, .w = cc, .h = 1, .c = c, .next = NULL};
      temp = cskel;
      while (1) {
        if (temp->c == c) {
          if (srow > temp->sy + temp->h - 1) {
            SKEL_ERROR("skipped row occurence for char '%c'; last "
                       "valid row occurence at row %d col %d new "
                       "one at row %zu col %zu\n",
                       c, temp->sy + temp->h - 1, temp->sx + temp->w - 1, srow,
                       scol);
          }
          goto NEXT;
        }
      ...
      string_destroy(str3);
      if (cc != ca)
        SKEL_ERROR("skipped occurence; consecutive row occurence of char "
                   "%c is %d but total occurence is %d along col %zu\n",
                   c, cc, ca, scol);
      temp->h = cc;
      if (cc != 1) {
        for (size_t l = temp->sy + 1; l < cc + temp->sy; ++l) {
          ...
          if (ll != temp->w || ll != lc)
            SKEL_ERROR("unequal col occurences for char '%c'; %d occurences "
                       "in row %d but %d occurence in row %zu\n",
  ...
  if (!cols || !row)
    SKEL_ERROR("string contains only spaces\n");
  ...
  cs -= unique_chars;
  return (Skel){
      .len = unique_chars, .rows = row, .cols = cols, .skel = ld, .labels = cs};
}
...
static void figure_draw_cursors(Figure *figure) {
  ...
  for (size_t i = 0; i < figure->axes_len; ++i) {
    ...
    DrawLine(zdata, axes->startY, zdata, axes->startY + axes->height,
             figure->text_color);
    zdata = ydata * axes->height + axes->startY;
    DrawLine(axes->startX, zdata, axes->startX + axes->width, zdata,
             figure->text_color);
  }
}
...
static bool set_real_span_skel_map(Figure *figure) {
  ...
  case SCREEN_DIMENSION_STATE_CHANGED:
    ...
    long int x =
        (long int)figure->height -
        (figure->font_size *
         (figure->title == NULL ? 1 : 2)); // may underflow if height<font_size
    if (x < 0) {
      return false;
    }
    ...
    for (size_t axes_index = 0; axes_index < figure->axes_len; axes_index++) {
      ...
      axes_skels_copy[1] = axes_sizes[1] * figure->axes_skels[index + 1] +
                           (figure->title ==
                            NULL ? 0 : figure->font_size);
...
static bool set_borders(Figure *figure) {
  figure->border_dimensions[0] =
      (figure->border_percentage * figure->width) / (figure->cols + 1);
  long int x = (long int)figure->height -
               (figure->font_size * (figure->title == NULL ? 1 : 2));
  if (x < 0) {
    return false;
  }
  figure->border_dimensions[1] =
      (figure->border_percentage * x) / (figure->rows + 1);
  return true;
}

void raylib_init(Figure *figure) {
  RC_ASSERT(figure->sds == SCREEN_DIMENSION_STATE_DEFAULT,
            "show can only be called once\n");
  figure->sds = SCREEN_DIMENSION_STATE_CHANGED;
  SetConfigFlags(FLAG_WINDOW_RESIZABLE);
  SetConfigFlags(FLAG_MSAA_4X_HINT);
  SetTraceLogLevel(LOG_ERROR);
  char *window_title = figure->window_title
                           ? figure->window_title
                           : string_create_from_format(
                                 0, NULL, "%s.%d", RAY_WINDOW_TITLE, getpid());
...
#define BUFFER_LEN 40
  ...
  string_append(buffer, "%04d-%02d-%02d   %02d:%02d:%02d   %02ld",
                ...
                tv.tv_usec / 10000);
  float x =
      align_text(FIGURE_FONT(figure), buffer, figure->width - 10,
                 figure->font_size, figure->font_spacing, RC_ALIGNMENT_CENTER);
  DrawTextEx(FIGURE_FONT(figure), buffer, (Vector2){x, figure->height / 2},
             figure->font_size, figure->font_spacing, figure->text_color);
}

static void draw_title(Figure *figure) {
  RC_ASSERT(figure->title != NULL);
  float x =
      align_text(FIGURE_FONT(figure), figure->title, figure->width,
                 figure->font_size, figure->font_spacing, RC_ALIGNMENT_CENTER);
  DrawTextEx(FIGURE_FONT(figure), figure->title, (Vector2){x, 0},
             figure->font_size, figure->font_spacing, figure->text_color);
}
...
#define buf_size 128
  ...
  if (figure->has_dragger) { // the user has called `set_dragger` because
    ...
    if (figure->axes_len != axes_under_mouse && axes->artist_len != 0) {
      int mousex_iloc = (int)(((float)(GetMouseX() - axes->startX) /
                               (axes->width) * figure->dragger.vlen) +
                              figure->dragger.start);
      string_append(buffer, "%zu [%d/%zu] ", figure->dragger.vlen, mousex_iloc,
                    (figure->dragger._len - 1));
      ...
      formatter_to_str(figure->dragger.locator.ftype,
                       figure->dragger.locator.format, buffer,
                       &figure->dragger.xdata[figure->dragger._len - 1]);
    }
  }
  float x =
      figure->border_dimensions[0] + AXES_FRAME_THICK +
      align_text(FIGURE_FONT(figure), buffer,
                 (size_t)figure->width -
                     (figure->border_dimensions[0] + AXES_FRAME_THICK),
                 figure->font_size, figure->font_spacing, RC_ALIGNMENT_LEFT);
  DrawTextEx(FIGURE_FONT(figure), buffer,
             (Vector2){x, figure->height - figure->font_size},
             figure->font_size, figure->font_spacing, figure->text_color);
}
...
bool update_figure(Figure *figure) {
  ...
  figure->sds =
      (sd[0] == figure->width && sd[1] == figure->height)
          ? SCREEN_DIMENSION_STATE_UNCHANGED
          : SCREEN_DIMENSION_STATE_CHANGED; // cannot be
...
void update_xlim(Figure *figure) {
  ...
  lmax += figure->dragger.timeframe / 2.f; // if visible_data=1 then
                                           // lmax-lmin=0
  figure->dragger.locator.limit =
      (Limit){.limit_max = lmax,
              .limit_min = lmin,
              .diff = lmax - lmin,
              .is_static = figure->dragger.locator.limit.is_static};
...
Figure *create_figure(char *figskel, int *fig_size, char *window_title,
                      Color background_color, float border_percentage, int fps,
                      size_t font_size, int font_spacing, char *font_path) {
  Skel s = skel_create(figskel);
  fas_print(fas_parse(figskel));
  

  if (border_percentage < 0.f || border_percentage >= 1.f) {
    RC_WARN("border_percentage is %f\n", border_percentage);
  }
  if (font_size == 0) {
  ...
  size_t *border_dimensions =
      cm_malloc(sizeof(size_t) * 2, RC_ECHO(border_dimensions));
  size_t *axes_skels_dyn =
      cm_malloc(sizeof(size_t) * s.len * 4, RC_ECHO(axes_skels_dyn));
  ...
  *figure = (Figure){
      ...
      .title = NULL,
      .window_title =
          window_title ? string_create_from_format(0, NULL, "%s", window_title)
                       : NULL,
      .axes_len = s.len,
      .rows = s.rows,
      .cols = s.cols,
...
void set_dragger(Figure *figure, size_t len, size_t timeframe, double *xdata,
                 FormatterType ftype, char *format) {
  if (figure->has_dragger) {
  ...
  dragger.ulen = RC_UPDATE_LEN > RC_INITIAL_VISIBLE_DATA
                     ? RC_INITIAL_VISIBLE_DATA - RC_UPDATE_LEN
                     : RC_UPDATE_LEN;
  ...
  if (format == NULL && ftype != FORMATTER_NULL_FORMATTER) {
    RC_ERROR("'%s' is NULL but formatter '%s' is not '%s'\n", RC_ECHO(format),
             RC_ECHO(FormatterType), RC_ECHO(FORMATTER_NULL_FORMATTER));
  }
  dragger.locator.ftype = ftype;
  dragger.locator.format =
      string_create_from_format(0, NULL, "%s", format ? format : "[NULL]");
  dragger.xdata_shared = cm_malloc(RC_MAX_PLOTTABLE_LEN * sizeof(double),
                                   RC_ECHO(dragger.xdata_shared));
  for (size_t i = 0; i < figure->axes_len; ++i) {
    figure->axes[i].xdata_buffer =
        cm_malloc(RC_MAX_PLOTTABLE_LEN * sizeof(double),
                  RC_ECHO(figure->axes.xdata_buffer));
  }
