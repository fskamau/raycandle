...
void locator_update_data_buffers(Axes *axes, LimitChanged lim) {
  if (lim == LIMIT_CHANGED_ALL_LIM) {
    RC_ASSERT(axes->artist_len != 0 && axes->artist != NULL);
  }
  size_t rows = axes->parent->dragger.vlen;
  if (rows == 0) {
    RC_ERROR("unreachable function '%s'  when figure has no xdataa\n",
...
void epoch2strftime(int epoch, Str buffer, const char *format) {
  ...
  written = strftime(string_get_current_point(buffer),
                     (len_buffer = string_get_remaining(buffer)), format, &tm);
  if (written == 0) {
...
*/
void locator_tooltip_mouse_position(Axes *axes, Str buffer, int mouseX,
                                    int mouseY) {
  RC_ASSERT(axes->artist_len != 0 && axes->parent->has_dragger != 0);
  if (axes->ylocator.format == NULL &&
      axes->ylocator.ftype != FORMATTER_NULL_FORMATTER) {
    RC_ERROR("formatter is not null but format is null\n");
  }
  double x = RC_PIXEL_X_2_DATA((float)mouseX, axes);
  double y = RC_PIXEL_Y_2_DATA((float)mouseY, axes);
  string_append(buffer, "x=");
  formatter_to_str(axes->parent->dragger.locator.ftype,
                   axes->parent->dragger.locator.format, buffer, &x);
  string_append(buffer, " y=");
  formatter_to_str(axes->ylocator.ftype, axes->ylocator.format, buffer, &y);
}

void formatter_to_str(FormatterType formatter, Str format, Str buffer,
                      void *mem) {
  switch (formatter) {
