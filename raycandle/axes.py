...
class Axes(RC_Axes):
    ...
    @window_not_closed
    def plot(
        self, artist: Union[Type[RC_Artist], pd.Series], **kwargs
    ) -> Type[RC_Artist]:
        """
        ...
        artist.__artist__ = self._rc_api.lib.create_artist(
            self._rc_api.fig.axes + self._id, *args
        )
    ...
    def set_xlim(self, *args, **kwargs) -> NoReturn:
        raise NotImplementedError(
            "all axes share same x-axis for now. to update all axes xlim, update from a point using Figure.update_from_position"
        )

    @final
    @window_not_closed
    def grid(
        self, xlen: int = 3, ylen: int = 3, color=(0, 0, 0, 255), lw=0.3, **kwargs
    ) -> None:
        [
            [
                func(0 if x == 0 else float(x) / len_, color=color, lw=lw, **kwargs)
                for x in range(len_)
            ]
    ...
    @window_not_closed
    def show_legend(
        self, legend_position: LegendPosition = LegendPosition.BOTTOM_LEFT
    ) -> None:
        self._rc_api.lib.axes_show_legend(
            self._rc_api.fig.axes + self._id, legend_position
        )
